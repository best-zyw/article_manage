...
<script>
  export default {
    ...
    created(){
        ...
        this.axios.get(`http://localhost:3000/categories/${id}`).then(res=>{
            console.log(res)
            this.ruleForm =res.data.data.category;
            this.ruleForm.show=!!res.data.data.category.show
        })
    ...
    methods: {
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.axios.put(`http://localhost:3000/categories/${this.$route.params.id}`,this.ruleForm).then(res=>{
                console.log(res)
                this.$message({
