  ...
  <div class="detail">
    ...
    <el-form ref="form" :model="form" :inline="true">
      ...
      <el-form-item>
       ...
       <el-form-item>
        <el-button type="primary" @click="new_">新建</el-button>
      </el-form-item>
    </el-form>
  ...
  methods: {
      ...
      handleDelete(index, row) {
        ...
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          type: 'warning'
            ...
            console.log(row)
           this.axios.delete(`http://localhost:3000/articles/${row.id}`).then(res=>{
                   this.init();
              })
          this.$message({
            type: 'success',
    ...
    onSubmit() {
      ...
      this.page.current_page=1
        this.axios.get(`http://localhost:3000/articles?current_page=${this.page.current_page}&categoryId=${this.new_form.categoryId}&title=${this.new_form.title}`).then(res=>{
    ...
    init() {
      this.axios
        .get(`http://localhost:3000/articles?current_page=${this.page.current_page}&categoryId=${this.$route.params.id}`)
        .then((res) => {
          console.log(res);
          let Data=res.data.data
          this.tableData=Data.articles
...
.detail .el-range-editor.el-input__inner{
  width: 350px;
}
 .detail .el-pagination{
  ...
  .detail .el-table td, .el-table th{
    text-align: center;
  }
